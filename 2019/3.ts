...
const wire1 = rawInput3.split("\n")[0].split(",");
const wire2 = rawInput3.split("\n")[1].split(",");
...
for (var i = 0; i < wire1.length; i++) {
    const direction = wire1[i].slice(0, 1);
    const steps = parseInt(wire1[i].slice(1));
    ...
    for (var j = 0; j < steps; j++) {
        let x = currentX + (changeXBy * j);
        let y = currentY + (changeYBy * j);
        arr1[`${x},${y}`] = arr1[`${x},${y}`] || currentSteps;
        currentSteps++;
    }
    currentX = currentX + (changeXBy * j);
    currentY = currentY + (changeYBy * j);
    arr1[`${currentX},${currentY}`] = currentSteps;
}
...
for (var i = 0; i < wire2.length; i++) {
    const direction = wire2[i].slice(0, 1);
    const steps = parseInt(wire2[i].slice(1));
    ...
    for (var j = 0; j < steps; j++) {
        let x = currentX + (changeXBy * j);
        let y = currentY + (changeYBy * j);
        arr2[`${x},${y}`] = currentSteps;
        currentSteps++;
    }
    currentX = currentX + (changeXBy * j);
    currentY = currentY + (changeYBy * j);
    arr2[`${currentX},${currentY}`] = currentSteps;
}
...
for (const key in arr1) {
    if (arr1.hasOwnProperty(key)) {
        if (arr2[key]) {
            connections.push({ key, cost: arr1[key] + arr2[key] });
        }
    }
}

const sortedConnectionsByManhattanDistance = connections.map(c => c.key.split(",").map(x => parseInt(x)).reduce((a, v) => a + Math.abs(v), 0)).sort((a, b) => a - b);
console.log(sortedConnectionsByManhattanDistance[1]);

const sortedConnectionsByTotalLength = connections.sort((a, b) => a.cost - b.cost);
console.log(sortedConnectionsByTotalLength[0].cost);
