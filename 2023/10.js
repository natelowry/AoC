var input = `FL7L-F7FJF-7FFF7FF|-F-F|JF--|.|-J-7.F--FJF|-7.F.-FF7.-F7-L-|FF-F-|-7.-F7-FF-|7J-J.F-J7.FF7..L7F|7F-7JF-L7F.|--FL7F---|.FFF-L7FL--L777L-.FF7F
...
function followThePipeAroundThisNode(current) {
  const ogX = current.x;
  const ogY = current.y;
  var currentStep = 0;
  ...
  } else if (["-"].includes(current.char)) {
    entryDir = Directions.W;
  }
...
function getOffsets(char, enteringFrom) {
  switch (char) {
    case "|": //vert
      return enteringFrom === Directions.N ? [0, 1, Directions.N] : [0, -1, Directions.S];
    case "-": //horizontal
