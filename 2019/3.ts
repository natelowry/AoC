...
const sortedConnections = connections.map(c => c.split(",").map(x => parseInt(x)).reduce((a, v) => a + Math.abs(v), 0)).sort((a, b) => a - b);
console.log(sortedConnections[1]);