...
// Your puzzle answer was 2171728567.
...
while (opcode9 !== 99) {
    const current = input9a[Number(index9)];
    //0 == position mode
    //1 == immediate mode
    //2 == relative mode
    // thirdparametermode secondparametermode firstparametermode 2digitopcode
    opcode9 = Number(current) % 100;
    const firstParamDigit = Math.floor(Number(current) / 100) % 10;
    const secondParamDigit = Math.floor(Number(current) / 1000) % 10;
    const thirdParamDigit = Math.floor(Number(current) / 10000) % 10;
    // console.log(thirdParamDigit);

    let firstParam = input9a[Number(index9) + 1];
    let secondParam = input9a[Number(index9) + 2];
    let thirdParam = input9a[Number(index9) + 3];

    ///////  console.log("current:", current, "firstParam:", firstParam, "secondParam:", secondParam, "base:", base9);

    //console.log(firstParamDigit, secondParamDigit);
    switch (firstParamDigit) {
        case 0:
            ///////         console.log("param 1 index: " + firstParam + " value: " + input9a[firstParam]);
            firstParam = BigInt(input9a[firstParam] || BigInt(0));
            break;
        case 1:
            firstParam = firstParam;
            break;
        case 2:
            ///////         console.log("param 1 index: " + firstParam + " + " + base9 + " value: " + input9a[firstParam + base9]);
            firstParam = BigInt(input9a[firstParam + base9] || BigInt(0));
            break;
    }

    switch (secondParamDigit) {
        case 0:
            ///////         console.log("param 2 index: " + secondParam + " value: " + input9a[secondParam]);
            secondParam = BigInt(input9a[firstParam] || BigInt(0));
            break;
        case 1:
            secondParam = secondParam;
            break;
        case 2:
            ///////          console.log("param 2 index: " + secondParam + " + " + base9 + " value: " + input9a[secondParam + base9]);
            secondParam = BigInt(input9a[secondParam + base9] || BigInt(0));
            break;
    }

    // if (current === 1102) {
    /////////   console.log("now    :", current, "firstParam:", firstParam, "secondParam:", secondParam, "base:", base9);
    // }
    ...
    if (opcode9 === 1) {
        ...
        if (thirdParamDigit === 2) {
            input9a[thirdParam + base9] = BigInt(firstParam) + BigInt(secondParam);
        } else {
            input9a[thirdParam] = BigInt(firstParam) + BigInt(secondParam);
        }
        ...
        //console.log(current, firstParam, secondParam, input9a[thirdParam], BigInt(firstParam) * BigInt(secondParam), input9a[input9a[Number(index9) + 3]]);
        index9 = Number(index9) + 4;
    } else if (opcode9 === 3) {
        //Opcode 3 takes a single integer as input and saves it to the position given by its only parameter. For example, the instruction 3,50 would take an input value and store it at address 50.
        if (firstParamDigit === 2) {
            input9a[input9a[Number(index9) + 1] + base9] = input9;
        } else {
        ...
        if (firstParam < secondParam) {
            if (thirdParamDigit === 2) {
                input9a[thirdParam + base9] = 1;
            } else {
                input9a[thirdParam] = 1;
            }
        } else {
            if (thirdParamDigit === 2) {
                input9a[thirdParam + base9] = 0;
            } else {
                input9a[thirdParam] = 0;
            }
        }
        index9 = Number(index9) + 4;
    } else if (opcode9 === 8) {
