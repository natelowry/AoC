var input = `............409..........784...578...802......64..............................486.248..............177....................369...............
  ...
  for (let x = 0; x < row.length; x++) {
    ...
    for (let xx = Math.max(0, x - 1); xx <= Math.min(row.length - 1, x + 1); xx++) {
      for (let yy = Math.max(0, y - 1); yy <= Math.min(arr.length - 1, y + 1); yy++) {
        ...
        if (Number.isNaN(parseInt(check)) && check != ".") {
          touchesASymbol[y][x] = 1;
        }
      }
    }
  }
...
for (let y = 0; y < arr.length; y++) {
  const row = arr[y];
  var currentNumber = [];
  var weHitASymbol = false;
  for (let x = 0; x < row.length; x++) {
    const char = row[x];

    var parsedInt = parseInt(char);
    if (!Number.isNaN(parsedInt)) {
      if (touchesASymbol[y][x] == 1) {
        weHitASymbol = true;
      }
     // console.log(`pushing: ${parsedInt} ${weHitASymbol}`);
      currentNumber.push(parsedInt);
    } else {
      //hit a non-number
      if (weHitASymbol && currentNumber.length > 0) {
